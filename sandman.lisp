...
(defun solve-cers-or-dump ()
  ...
  (dolist (i (copy-list *solved-edges*))
    (dolist (j *cers-rm-edges*)
      (when (and (is-vertex-edge (first j) (first i))
		 (is-vertex-edge (second j) (first i)))
	(setf *solved-edges* (remove i *solved-edges*)))))
...
(defun start (&key call-cers debug)
  (let ((*cers-solutions* nil)
    ...
    (setf *debug* debug)
    (when call-cers
      (multiple-value-bind (solutions edges rm-edges)
	  (archa-solve)
	(setf *cers-edges* edges)
	(setf *cers-rm-edges* rm-edges)
